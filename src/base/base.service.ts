...
export class BaseService<T extends ObjectLiteral> {
  ...
  async findAll(): Promise<T[]> {
    try {
      ...
      this.logger.error(
        `Error fetching all ${this.entityName}:`,
        error.stack || error.message,
      );
      throw new InternalServerErrorException(
  ...
  async findAllAdvanced(options?: QueryOptions<T>) {
    try {
      ...
      this.logger.error(
        `Error fetching ${this.entityName} list:`,
        error.stack || error.message,
      );
      throw new InternalServerErrorException(
        `Failed to fetch ${this.entityName} list`,
  ...
  async findOne(id: number): Promise<T> {
    try {
      ...
      this.logger.error(
        `Error fetching ${this.entityName} ID ${id}:`,
        error.stack || error.message,
      );
  ...
  async update(id: number, data: QueryDeepPartialEntity<T>): Promise<T> {
    try {
      ...
      this.logger.error(
        `Error updating ${this.entityName} ID ${id}:`,
        error.stack || error.message,
      );
      if (
  ...
  async remove(id: number) {
    try {
      ...
      this.logger.log(`🗑️ Deleted ${this.entityName} with ID ${id}`);
      return { message: `${this.entityName} deleted successfully` };
    } catch (error) {
      this.logger.error(
        `Error deleting ${this.entityName} ID ${id}:`,
        error.stack || error.message,
      );
      if (error instanceof NotFoundException) throw error;
      throw new InternalServerErrorException(
